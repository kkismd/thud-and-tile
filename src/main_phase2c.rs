...
use thud_and_tile::cli::{
    CliGameState, CliInputHandler, CliRenderSettings, CliRenderer, CoreGameEvent,
};
...
fn main() -> io::Result<()> {
    ...
    while frame_count < max_frames {
        ...
        // ===== 描画処理 (Phase 2C) =====
        let render_needed =
            cli_state.needs_redraw() || last_render_time.elapsed() >= Duration::from_millis(100); // 最低10 FPS
        ...
        if frame_count % 100 == 0 && frame_count > 0 {
            ...
            println!(
                "📊 Phase 2C統計 - 実行時間: {:.1}s, フレーム: {}, 入力: {}, イベント: {}",
