...
use crate::animation::{count_solid_lines_from_bottom, determine_erase_line_count, consume_chain_bonus_for_erase_line};
...
fn simulate_erase_line_completion(state: &mut GameState, _time_provider: &MockTimeProvider) {
    ...
    if let Some(Animation::EraseLine { target_solid_lines, .. }) = 
        state.animation.iter().find(|anim| matches!(anim, Animation::EraseLine { .. })) {
...
fn test_erase_line_completion_spawns_new_piece() {
    ...
    assert!(state.current_piece.is_none(), "EraseLineアニメーション中は新ピース未生成");
...
fn test_no_new_piece_with_other_animations_running() {
    ...
    if let Some(Animation::EraseLine { target_solid_lines, .. }) = 
        state.animation.iter().find(|anim| matches!(anim, Animation::EraseLine { .. })).cloned() {
