...
use crate::core::animation_logic::{
    should_render_line_with_animation, AnimationState, AnimationType,
};
...
impl CliAnimationManager {
    ...
    pub fn get_animation_intensity(
        &self,
    ...
    fn update_visibility_cache(&mut self, core_state: &CoreGameState, current_time_ms: u64) {
        ...
        let active_animations: Vec<AnimationState> =
            core_state.animations[..core_state.animations_count].to_vec();

        for line_y in 0..self.cached_visible_lines.len() {
            self.cached_visible_lines[line_y] =
                should_render_line_with_animation(line_y, &active_animations, current_time_ms);
        }
    }
