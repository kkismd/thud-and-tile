...
/// テスト用のボード作成ヘルパー
fn create_test_board_with_bottom_solid_lines(solid_lines: usize) -> [[Cell; BOARD_WIDTH]; BOARD_HEIGHT] {
...
fn test_piece_lock_to_chain_bonus_increase() {
    ...
    assert!(matches!(state.board[19][4], Cell::Connected { color: GameColor::Cyan, count: 5 }));
    assert!(matches!(state.board[19][5], Cell::Connected { color: GameColor::Cyan, count: 5 }));
    assert!(matches!(state.board[19][6], Cell::Connected { color: GameColor::Cyan, count: 5 }));
    assert!(matches!(state.board[19][7], Cell::Connected { color: GameColor::Cyan, count: 5 }));
    assert!(matches!(state.board[19][8], Cell::Connected { color: GameColor::Cyan, count: 5 }));
    
    // chain_bonusが増加したかをチェック（初期値が0の場合、最大でも同じになることがある）
    if state.max_chains.cyan > old_max_chains.cyan {
        assert!(state.chain_bonus >= initial_chain_bonus, 
               "chain_bonus should increase or stay same: {} to {}", initial_chain_bonus, state.chain_bonus);
    }
}
...
fn test_enable_erase_line_triggers_animation() {
    ...
    assert_eq!(target_lines.len(), 2, "2つのターゲットラインが設定されるべき");
}
...
fn test_chain_bonus_consumption_for_erase_line() {
    ...
    let (remaining_bonus, consumed) = consume_chain_bonus_for_erase_line(initial_chain_bonus, lines_to_erase);
    
    assert_eq!(consumed, lines_to_erase, "消費されるchain_bonusは除去ライン数と等しい");
    assert_eq!(remaining_bonus, initial_chain_bonus - lines_to_erase, "残りのchain_bonusが正確");
}

#[test]
fn test_determine_erase_line_count_logic() {
    // 十分なchain_bonusとSolidライン
    assert_eq!(determine_erase_line_count(10, 5), 5, "Solidライン数がchain_bonusより少ない場合");
    
    // chain_bonusが不足
    assert_eq!(determine_erase_line_count(3, 5), 3, "chain_bonusが不足している場合");
    
    // 境界値
    assert_eq!(determine_erase_line_count(5, 5), 5, "chain_bonusとSolidライン数が等しい場合");
...
fn test_solid_line_removal_integration() {
    ...
    for x in 0..BOARD_WIDTH {
        assert!(matches!(new_board[BOARD_HEIGHT - 1][x], Cell::Solid), 
               "シフト後も底部はSolidライン");
        assert!(matches!(new_board[0][x], Cell::Empty), 
               "最上部は空行になる");
    }
...
fn test_full_erase_line_animation_flow() {
    ...
    assert_eq!(target_lines.len(), initial_solid_count, "Solidライン数と一致するターゲットライン");
    ...
    // 3. chain_bonus消費
    let (_remaining_bonus, consumed) = consume_chain_bonus_for_erase_line(state.chain_bonus, lines_to_erase as u32);
    state = state.consume_chain_bonus(consumed).0;
    assert_eq!(consumed, 3, "3のchain_bonusが消費される");
    assert_eq!(state.chain_bonus, initial_chain_bonus - 3, "chain_bonusが正確に減少");
    ...
    // ボード最下部が空になっている
    for x in 0..BOARD_WIDTH {
        assert!(matches!(current_board[BOARD_HEIGHT - 1][x], Cell::Empty), 
               "最下部は空セルになる");
    }
}
...
fn test_erase_line_with_insufficient_chain_bonus() {
    ...
    assert_eq!(lines_to_erase, 2, "chain_bonusが不足する場合は利用可能分のみ");
    
    // chain_bonus消費
    let (_, consumed) = consume_chain_bonus_for_erase_line(state.chain_bonus, lines_to_erase as u32);
...
fn test_erase_line_disabled_scenario() {
    ...
    assert!(!should_start, "enable_erase_lineが無効の場合はアニメーション開始しない");
}