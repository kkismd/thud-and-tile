...
**フェーズ2: SRS対応のためのデータ構造の導入と `SHAPES` の再定義 (完了済み)**
...
**目的:** SRSウォールキックデータを正確に定義し、回転ロジックで利用できるようにする。

1.  **Red (3.1): SRSウォールキックデータ構造の定義とテスト (完了済み)**
2.  **Green (3.2): SRSウォールキックデータの実装 (完了済み)**
3.  **Red (3.3): 残りのSRSウォールキックデータのテスト追加 (完了済み)**
4.  **Green (3.4): 残りのSRSウォールキックデータの実装 (完了済み)**
5.  **Refactor (3.5): ウォールキックデータ構造の最適化 (完了済み)**

**フェーズ4: SRS回転ロジックの実装と色の整合性 (改訂版)**
...
3.  **Red (4.3): ウォールキック適用ロジックのテスト**
    ...
    *   **テスト:** `src/tests/rotation_tests.rs` に、以下のシナリオをカバーする統合テストを追加し、失敗することを確認する。
        *   壁際でウォールキックが成功するシナリオ（例: Iミノの壁際回転）。
        *   ウォールキックを試みても回転が不可能なシナリオで回転が拒否されること。
        *   Oミノがウォールキックを適用しないこと。

4.  **Green (4.4): ウォールキック適用ロジックの実装**
    *   `Tetromino::rotated()` および `Tetromino::rotated_counter_clockwise()` メソッドに、`src/srs.rs` で定義した `WALL_KICKS` データを使用してウォールキックを適用するロジックを実装する。
    *   **テスト:** 上記のテストがパスすることを確認する。

5.  **Refactor (4.5): コードの可読性向上**
    *   回転ロジックのコードが明確で理解しやすいことを確認する。必要であれば、プライベートヘルパー関数をさらに分割したり、コメントを追加したりする。

**フェーズ5: 回転時のブロック色の整合性保証 (改訂版)**

**目的:** `Tetromino::rotated()` および `Tetromino::rotated_counter_clockwise()` メソッドにおいて、回転後のブロック位置と元のブロックの色の対応関係を正しく維持する。

1.  **Red (5.1): ブロック色の対応関係維持ロジックのテスト**
    *   **設計:** `Tetromino::rotated()` および `Tetromino::rotated_counter_clockwise()` メソッドが、回転後のブロックの相対位置に基づいて、元のブロックの色を正しく新しい位置に割り当てることを検証するテストを追加する。
    *   **テスト:** `src/tests/rotation_tests.rs` に、各テトリミノ（Oミノを除く）について、以下のシナリオをカバーするテストを追加し、失敗することを確認する。
        *   初期状態から時計回り・反時計回りに回転させた後、各ブロックの色が元のブロックの色と一致していること。
        *   特に、ブロックの相対位置が変化するテトリミノ（I, T, L, J, S, Z）について、回転前後でブロックの色が正しく追従していることを検証する。

2.  **Green (5.2): ブロック色の対応関係維持ロジックの実装**
    *   **設計:** `Tetromino::rotated()` および `Tetromino::rotated_counter_clockwise()` メソッド内で、回転後のブロックの相対位置と元のブロックの色の対応関係を正しく維持するロジックを実装する。
    *   **実装計画:**
        1.  `rotated()` および `rotated_counter_clockwise()` メソッド内で、`calculate_rotated_blocks_relative` から得られた新しいブロックの相対座標と、元のピースのブロックの色を関連付ける方法を検討します。
        2.  最もシンプルな方法は、元のピースの `blocks` フィールドのインデックス `i` に対応する色 `self.blocks[i].1` を、新しい相対座標 `new_blocks[i]` に割り当てることです。これは、`SRS_SHAPES` のブロック定義が、回転前後でブロックの論理的な順序を維持していることを前提とします。
        3.  `new_piece.blocks` を構築する際に、この対応関係を適用します。

3.  **Refactor (5.3): コードの可読性向上**
    *   **設計:** 色の対応関係を維持するロジックが明確で理解しやすいことを確認する。必要であれば、ヘルパー関数やコメントを追加する。
    *   **実装計画:**
        1.  色の対応関係を維持するロジックが複雑になりすぎないように、必要に応じてプライベートヘルパー関数に抽出することを検討します。
        2.  コードの意図を明確にするために、適切なコメントを追加します。
        3.  変数名や関数名が、その役割を正確に表しているかを確認し、必要に応じて修正します。
