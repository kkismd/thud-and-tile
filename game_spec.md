...
**サイクル 1: `calculate_connected_blocks` 関数の基本構造と単一ブロックの処理**

1.  **Red (テストの追加)**
    *   **テストケース:** 空の盤面または単一の固定ブロックがある盤面を`calculate_connected_blocks`関数に渡した場合、パニックせず、`Cell`の`connection_count`が`None`のままであることを確認するテスト。
    *   **テストケース:** 盤面に固定ブロックが一つだけ存在し、その周囲に同色のブロックがない場合、`calculate_connected_blocks`関数実行後もそのブロックの`connection_count`が`None`のままであることを確認するテスト。
2.  **Green (最小限の実装)**
