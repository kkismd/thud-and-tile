...
*   **得点システム**
    1. 得点はブロック消去で加算されるSCOREと、連結ボーナスを示すCHAIN-BONUS、および色別のMAX-CHAINがある
    ...
    6. CHAIN-BONUSは連結グループボーナスで獲得したSolidライン消去可能段数を表す
        ...
        - 複数のグループがある場合は累積される
        - **上限は10段**：10段を超えて獲得することはできない
        - 上限を超える分は切り捨てられる（例：現在8段で3段獲得 → 10段に）
        - ライン消去アニメーション終了後、実際にSolidラインを消去する際に使用される
        - 使用後は残った分が持ち越される（例：10段保有、2段使用 → 8段残る）
...
```
    8. CHAIN-BONUSの表示仕様
        ...
        - 値に応じて色が変化する：
            ...
            - 8-9段：赤色
            - 10段（満タン）：濃い赤色 + `⚠` マーク
        - 満タン時の表示例：`CHAIN-BONUS: [██████████] 10/10 ⚠`（濃い赤色で固定表示）
