...
### リファクタリング計画: `main.rs` のボードロジックを `board_logic.rs` へ移動

- [ ] **1. `src/board_logic.rs` の作成:** ボード関連のゲームロジックをカプセル化するための新しいモジュールを作成します。
- [ ] **2. 関数の移動:** `find_and_connect_adjacent_blocks`、`count_connected_blocks`、`handle_scoring`、`remove_isolated_blocks` を `main.rs` から `src/board_logic.rs` へ移動します。
