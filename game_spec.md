...
*    新しいミノの色を以下のルールで4色から3色に減らしたい
    ...
    *   各テトリミノは4つのブロックで構成され、出現時にこれら3色が隣り合わないようにランダムに割り当てられる（1つのテトリミノ内で4つのブロックは異なる色を持つ）。
...
- [x] **Red:** 新しいテトリミノがシアン、マゼンタ、イエローの3色のみで構成されることを確認するテストを追加する。このテストは最初は失敗するはず。
- [x] **Green:** 色の選択ロジックを修正し、使用する色を3色に限定してテストをパスさせる。
- [x] **Red:** テトリミノの隣接するブロックが同じ色にならないことを確認するテストを追加する。各テトリミノ形状で、隣り合うブロックの色が異なることをアサートする。このテストは最初は失敗するはず。
- [x] **Green:** 隣接するブロックが同じ色にならないように色を割り当てるロジックを実装し、テストをパスさせる。
- [x] **Refactor:**
    - [x] `tetromino.rs`内の新しい色割り当てロジックをリファクタリングし、可読性と効率を向上させる。
    - [x] `cargo test`を実行し、すべてのテストがパスすることを確認する。
- [x] **手動確認:** ゲームを実行し、新しいテトリミノの配色が視覚的に正しいことを確認する。
