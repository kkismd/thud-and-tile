...
- README / game_spec.md を最新版仕様に追随

### ⚠ Push Downアニメーションの既知バグ
- 現象: 非底面ラインを複数同時にSolid化した際、Push Downで下がっていく過程で連結セルが残存することがある
- 原因: `result.completed_push_downs` 内の複数ラインを同時処理すると、ベクタ操作で行インデックスがずれ、後続ラインが誤った位置を参照する可能性がある
- 暫定結論: Push Downは常に1本ずつ（下から順に）処理するシーケンスへ変更する必要がある
- 対応方針: アニメーション制御の規模が大きいため、現フェーズでは記録のみ行い、CHAIN-BONUS関連のマージを優先する
